...
export default function DesktopViewer({
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        loader.load(
          ...
          (loadError) => {
            const errMessage = loadError instanceof Error ? loadError.message : 'Unknown error';
  ...
  useEffect(() => {
    ...
    // ✨ 2. 경고 해결: useEffect 실행 시점의 containerRef.current 값을 변수에 저장
    const currentContainer = containerRef.current;
    let resizeCleanup: (() => void) | undefined;
    
    initializeDesktop3D().then(cleanup => {
        resizeCleanup = cleanup;
    });

    return () => {
      console.log(`🧹 DesktopViewer 정리`);
      ...
      // ✨ 2. 경고 해결: cleanup 함수에서 저장해둔 변수 사용
      if (currentContainer) {
              ...
              <div className="space-y-3">
                <button 
                  ...
                  className={`w-full py-2 px-4 rounded-lg transition-all ${
                    copySuccess ? 'bg-green-500 text-white' : 'bg-blue-500 hover:bg-blue-600 text-white'
                  }`}
                >
