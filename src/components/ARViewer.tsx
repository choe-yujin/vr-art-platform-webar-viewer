...
export default function ARViewer({ 
  ...
  const loadARModel = async (scene: any, mindarThree: any) => {
    try {
      ...
      // three-icosa 확장자 등록 (AR용) - 안전한 로딩 전략
      try {
        ...
        if (GLTFGoogleTiltBrushMaterialExtension) {
          console.log('🔧 AR용 three-icosa 확장자 등록 시도...');
          // 확장자 등록을 안전하게 처리
          loader.register((parser: any) => {
            // parser 유효성 검증
            if (!parser || typeof parser !== 'object') {
              console.warn('⚠️ 유효하지 않은 parser 객체:', parser);
              return null;
            }
            
            try {
              // 1순위: 기본 애셋 경로
              return new GLTFGoogleTiltBrushMaterialExtension(parser);
            } catch (error) {
              console.warn('⚠️ three-icosa 기본 애셋 실패:', error);
              try {
                // 2순위: 로컬 브러시 경로
                return new GLTFGoogleTiltBrushMaterialExtension(parser, '/brushes/');
              } catch (fallbackError) {
                console.warn('⚠️ three-icosa 로컬 애셋도 실패:', fallbackError);
                return null;
              }
            }
          });
        ...
        console.warn('⚠️ AR용 three-icosa 로드 실패 (기본 GLTFLoader로 진행):', icosaError);
      }
  ...
  const loadDesktopModel = async (scene: THREE.Scene, camera: THREE.PerspectiveCamera, controls: OrbitControls) => {
    try {
      ...
      // three-icosa 확장자 등록 (데스크톱용) - 안전한 로딩 전략
      try {
        // @ts-ignore - three-icosa 모듈 로딩
        const threeIcosaModule = await import('three-icosa');
        const { GLTFGoogleTiltBrushMaterialExtension } = threeIcosaModule;
        ...
        if (GLTFGoogleTiltBrushMaterialExtension) {
          console.log('🔧 데스크톱용 three-icosa 확장자 등록 시도...');
          // 확장자 등록을 안전하게 처리
        ...
        console.warn('⚠️ 데스크톱용 three-icosa 로드 실패 (기본 GLTFLoader로 진행):', icosaError);
      }
